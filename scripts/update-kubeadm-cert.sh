...
cert::update_etcd_cert() {
  ...
  # restart etcd with Docker
  #docker ps | awk '/k8s_etcd/{print$1}' | xargs -r -I '{}' docker restart {} >/dev/null 2>&1 || true
  
  # restart etcd with Tanzu Kubernetes Grid
  crictl ps | awk '/etcd/{print$1}' | xargs -r -I '{}' crictl stop {} >/dev/null 2>&1 || true
...
cert::update_master_cert() {
  ...
  for item in "apiserver" "controller-manager" "scheduler"; do
    ...
    crictl ps | awk '/kube-'${item}'/{print$1}' | xargs -r -I '{}' crictl stop {} >/dev/null 2>&1 || true
